...
const std::string& Msg::selfCompress()
{
    if(m_compressed.empty()){
        std::ifstream file(m_FileName.c_str(), ios::binary);
        std::string inBuffer;
        file.seekg(0, std::ios::end);
        inBuffer.resize(file.tellg());
        file.seekg(0, std::ios::beg);
        file.read(&inBuffer[0], inBuffer.size());
        try{
            m_compressed = Utils::compress_string(inBuffer);
        }catch(const std::runtime_error& er){
            std::cout<<er.what();
        }
    }
    std::cout<<"Compressed data size: " << m_compressed.size() << std::endl;
    return m_compressed; 
}
